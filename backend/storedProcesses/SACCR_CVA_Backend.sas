...
data SACCR_ERROR_TABLE(
    keep = CVA_AS_OF_DATE
           CVA_CPTY_ID
           CVA_CPTY_NAME
           CVA_CPTY_TYPE_CODE
           CVA_NETTING_SET_ID
           CVA_PRIMARY_RISK_DRVR_CODE
           X_CUSTOM_RULE
);
    set ORC_LIB.SACCR_ERROR_TABLE(
        where=(datepart(CVA_AS_OF_DATE) = "&reporting_date."d)
    );
run;


/* ── Step 5: Write JSON to _webout ── */
proc json out=_webout pretty nosastags;
    ...
    /* Error records filtered by reporting date — rendered in Exception Report tab */
    write values "exception_report";
    export SACCR_ERROR_TABLE / nosastags;
